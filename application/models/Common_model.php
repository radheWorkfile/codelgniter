...
public function balance_sheet($where)
{
  ...
  // ==================================== List section start from here. ======================================= 
    public function showLIst_query($where=false){
        $i=0;$field = array('id,name,email,mobile');
        ...
        $data = $this->db->select('*')->from('employee');
        ...
        public function showLIst_model($where = false,$accs=false)
        {$this->showLIst_query($where,$accs);if ($where['length'] != -1) {$this->db->limit($where['length'], $where['start']);}return $this->db->get()->result();}
        public function total_count($accs=false){$this->showLIst_query($where = false,$accs);return $this->db->get()->num_rows();}
        public function total_filter_count($where = false,$accs=false){$this->showLIst_query($where,$accs);return $this->db->get()->num_rows();
        }
